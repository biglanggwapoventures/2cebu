<div class="card mb-2 {{ (isset($onMap)  && $onMap) ? 'attraction' : '' }}" data-latitude="{{ $item->latitude }}" data-longitude="{{ $item->longitude }}" data-title="{{ $item->name }}">
    <div style="height:200px;background:url('{{$item->thumbnail? $item->thumbnail->filepath : MyHelper::photoPlaceholder()}}') center center;background-size: cover" alt="{{ $item->name }}" class="image"></div>
     <div class="card-body p-3">
        ...
        <p class="card-text text-truncate clearfix location">{{ $item->location }}</p>
